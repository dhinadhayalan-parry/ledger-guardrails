...
**The gate fails closed.** An unknown environment is treated as `prod`, and a control missing from the catalog is enforced. Both cases are covered by tests.
...
|---|---|---|
| `id-ledger-gh-plan` | `repo:<owner>/ledger-guardrails:environment:plan` | Reader on the subscription, Storage Blob Data Reader on the state account |
...
The plan job runs with `-lock=false` because the plan identity cannot write the state lease.
...
| 1 | Management groups, Azure Policy definitions and initiative for the same controls, exemptions as code with `expires_on`, apply identity, self-hosted runner in the VNet | Planned |
...
- Tags that are unknown until apply cannot be checked at plan time (ADR 0002).
